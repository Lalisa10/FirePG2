...
Menu::Menu(SDL_Renderer* renderer){
       ...
       _restart_button->loadFromFile(renderer, "Restart.png");

       _resume_button = new Button ();
       _resume_button->loadFromFile(renderer, "Resume.png");

       _exit_button = new Button ();
       _exit_button->loadFromFile(renderer, "Exit.png");

       _final_score = new Text ("X.Template-font-stencil.ttf", 30);
       _highest_score = new Text ("X.Template-font-stencil.ttf", 30);
}
...
gameStatus Menu::handleInputAction(const SDL_Event& event, gameStatus currentStatus, Cursor* mouse){
       ...
       if(currentStatus != GAME_START && currentStatus != GAME_OVER) _resume_button->handleInputAction(event, mouse);
       ...
       else if(_resume_button->isClicked()){
              _resume_button->setClicked(false);
              return GAME_PLAYING;
       }
...
void Menu::update(gameStatus currentGameStatus){
       ...
       if(_alpha_mod < MAX_ALPHA){
              ...
              _resume_button->activate(true);
              _exit_button->activate(true);

       }

       _restart_button->setAlpha(_alpha_mod );
       _resume_button->setAlpha(_alpha_mod );
...
void Menu::draw(SDL_Renderer* renderer, gameStatus currentGameStatus){
       ...
       if(currentGameStatus == GAME_START || currentGameStatus == GAME_OVER){
              ...
              _resume_button->setRect( ( SCREEN_WIDTH - _resume_button->getRect().w) / 2, 300 );
              _resume_button->render(renderer);
