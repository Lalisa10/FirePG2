...
void gameMap::dfs_generate_random_maze(int y, int x){
       ...
       for(int i : direct_order){
              ...
              if(_game_map.tile[next_y][next_x] == STONE || _game_map.tile[next_y][next_x] == GRASS){
...
void gameMap::loadTileMap(){
       ...
       for(int i = 0; i < MAX_MAP_Y; i ++){
              for(int j = 0; j < MAX_MAP_X; j ++){
                   if(rand() % 5) _game_map.tile[i][j] = STONE;
                   else _game_map.tile[i][j] = GRASS;
              }
       }
